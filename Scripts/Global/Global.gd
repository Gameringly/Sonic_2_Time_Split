...
var Past = load("res://Scene/Zones/BaseZone.tscn")
var Future = load("res://Scene/Zones/BaseZone.tscn")
...
var ring_frame = 0 #sync ring animations
...
func _process(delta):
	...
	if !get_tree().paused:
		globalTimer += delta
		ring_frame = ring_frame + (10 * delta)
		if ring_frame >= 4:
			ring_frame = 0
